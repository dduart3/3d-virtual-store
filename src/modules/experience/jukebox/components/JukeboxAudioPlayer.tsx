...
export const JukeboxAudioPlayer = () => {
  ...
  // Volume reduction factor when outside the store
  ...
  useFrame(() => {
    ...
    if (audioRef.current.volume !== calculatedVolume) {
      audioRef.current.volume = THREE.MathUtils.lerp(
  ...
  useEffect(() => {
    ...
    const handleNowPlaying = (songData: any | null) => {
      ...
      if (songData && songData.filePath) {
          ...
          if (audioInitializedRef.current) {
            audioRef.current
              ...
              .then(() => {
                ...
                if (songData.startTime && audioRef.current) {
        ...
        audioRef.current.src = DEFAULT_SONG_URL;
        currentSrcRef.current = DEFAULT_SONG_URL;
    ...
    const handleSync = (data: any) => {
      ...
      if (data.song && data.song.filePath) {
        ...
        audioRef.current
          .play()
          .then(() => {
            if (data.elapsedTime > 0 && audioRef.current) {
              console.log(
                `Syncing: Seeking to ${data.elapsedTime}ms into the song`
              );
              audioRef.current.currentTime = data.elapsedTime / 1000;
            }
          })
  ...
  }, [socket, setCurrentSong]); // Only depends on socket and setCurrentSong
